...
parser.add_argument('--zip', '-z', dest='zip_s', nargs='?', const="True",
                    help="Zip archive the screenshot folder to main directory. Still call -rms if needed")
...
parser.add_argument('--extract-only', '-exto', dest='extract_only', nargs='?', const="True",
                    help="Don't do any screenshots and only extract the 1 sub track and fonts.")
...
if __name__ == '__main__':
    ...
    if remove_sources:
        remove_types = ('*.pgs', '*.ass', '*.tff', '*.otf', '*.sub', '*.idx', '*.srt')
        matching_files = []
        os.chdir(save_path)  # eh couldnt do it regular way
        for files in remove_types:
            matching_files.extend(glob.glob(files))
        for files in matching_files:
            os.remove(files)
        os.chdir("..")  # I hate this
        print("Removed sources")

    if to_zip:
        shutil.make_archive(os.path.splitext(os.path.basename(filename))[0], 'zip', save_path)
